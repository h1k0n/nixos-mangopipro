...
# /boot partition, and btrfs root partition. The generated image
...
in
{
  ...
  options.sdImage = {
    ...
    bootPartitionOffset = mkOption {
      ...
      default = 8;
      description = ''
        Gap in front of the /boot partition, in mebibytes (1024×1024
        bytes).
  ...
  config = {
    ...
    system.build.sdImage = pkgs.callPackage ({ stdenv, dosfstools, btrfs-progs,
    mtools, libfaketime, util-linux, zstd }: stdenv.mkDerivation {
      ...
      buildCommand = ''
        ...
        faketime "1970-01-01 00:00:00" mkfs.vfat -i ${config.sdImage.bootPartitionID} -n ${config.sdImage.bootPartitionName} boot_part.img
